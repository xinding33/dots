...
**Never animate keyboard-initiated actions.** Users repeat them often, so
animation makes them feel slow and disconnected. Raycast correctly omits its
...
Springs feel natural because they simulate physics. They settle according to
their parameters instead of using a fixed duration. Use them for momentum,
interruptible gestures, decorative pointer tracking, and elements that should
feel alive.
...
- **Momentum dismissal:** Do not require the gesture to cross a distance
  threshold. Compute velocity with `Math.abs(distance)/elapsedMs` and dismiss
  when it exceeds approximately `0.11`. A flick should be sufficient.
...
- **Multi-touch protection:** Ignore extra touch points after the drag begins
  with `if (isDragging) return`. This prevents jumps.
...
Reduced motion uses fewer and gentler animations. Keep transitions that aid
comprehension, but remove position changes.
...
Match motion to the component's personality. Playful components can use more
bounce, while professional dashboards should feel crisp. Sonner uses slightly
slower `ease` motion to match its visual design. Tune opacity and height
together for entering or exiting lists.
