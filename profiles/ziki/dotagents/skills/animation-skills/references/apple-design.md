...
- **Feedback must be continuous *during* the interaction, not just at the end.** For a drag, slider, or drawer, update the UI 1:1 with the pointer throughout the gesture.
...
- **Response:** Controls how quickly the value reaches the target. Lower values
  feel faster. Response is not duration because a spring has no fixed duration.
...
6. **Simplicity.** Remove what the core purpose does not need. Use plain
   language, fewer steps, and clear hierarchy. Adding context can simplify a
   task when it answers a necessary question. Show the common path first and
   put advanced options one level deeper.
7. **Craft.** Use deliberate typography, color, iconography, spacing, timing,
   and alignment. Adapt colors to light and dark modes. Use responsive motion
   for immediate feedback. Fix jitter, misalignment, and layouts that break
   during rotation. Continue refining the design as features and hardware
   change.
