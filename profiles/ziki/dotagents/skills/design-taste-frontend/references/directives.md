...
* SVG icons from libraries: fine (see `architecture.md` 3.C).
...
* **Fake-precise numbers are flagged.** Numbers like `92%`, `4.1×`, `48k`, `5.8 mm`, `13.4 lb` either:
  - Accept values from real data, including the brief, brand guidelines, and
    public metrics.
  - Accept values explicitly labeled as mock data with `<!-- mock -->`,
    "example", or "sample data".
  - Reject invented specification values. Do not imply engineering precision
    that the brand does not claim.
...
* **No em-dashes inside the quote text** as design flourish (long pauses, kinetic em-dashes, em-dash-bullets). See `ai-tells.md` 9.G: em-dash is completely banned.
* Attribution: name + role + (optionally) company. Never name only ("- Sarah").
* Quote marks: use plain ASCII quotes or no quote marks.
