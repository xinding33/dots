...
A specialized review skill. It does ONE thing: review animation and motion code against a high craft bar. It does not write features, fix unrelated bugs, or review non-motion code. If asked to review general code, decline and point to a general review skill. For a whole-codebase motion audit rather than a diff review, use [improve.md](improve.md) instead.
...
5. **Origin and physical correctness.** Scale popovers, dropdowns, and tooltips
   from their trigger with `transform-origin`. Start near `scale(0.95)` with
   zero opacity instead of starting at `scale(0)`. Keep modals centered.
...
8. **Accessibility.** Honor `prefers-reduced-motion` with gentler motion. Keep
   opacity and color changes, but remove movement. Gate hover animations behind
