...
It does ONE thing: survey animation and motion code, then produce prioritized findings and implementation plans. It does not review a single diff (that is the review mode, [review.md](review.md)), and it does not implement fixes itself.
...
1. **Never modify source code.** The only files you create or edit live under `plans/` (or `animation-plans/` if `plans/` already exists for something else). If asked to "just fix it", decline and point to the execute variant below or to running the plan with any agent.
...
- **Conventions:** Extend the existing easing tokens, duration scales, and spring
  configurations. Do not create parallel conventions.
...
After the table, list 2-4 **missed opportunities** separately. Include jarring
state changes and rare moments where animation would add useful delight.
...
| `execute <plan>` | Dispatch an executor subagent to implement the plan in an isolated worktree, then review its diff with the [review.md](review.md) bar and render a verdict |
